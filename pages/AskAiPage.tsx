...
const generateMockPlan = (goal: string): NutritionPlan => {
    ...
    } else if (goal.toLowerCase().includes('building')) {
        planData = {
...
const AskAiPage: React.FC<AskAiPageProps> = ({ onClose, userMeasurements, additionalDietItems, onSavePlan, source }) => {
    const [step, setStep] = useState<AiStep>(source === 'journal' ? 'dateSelection' : 'promptSelection');
    ...
    const handleDateSelectionContinue = (start: string, end: string) => {
        setStartDate(start);
        setEndDate(end);
    ...
    const handleBack = () => {
        if (step === 'result') {
            setStep('promptSelection');
        } else if (step === 'promptSelection') {
            if (source === 'journal') {
                setStep('dateSelection');
            } else {
                onClose();
            }
        } else { // dateSelection or loading (though loading shouldn't have a back button)
            onClose();
        }
    };
    
    const SelectDateRangeStep: React.FC<{ onContinue: (start: string, end: string) => void }> = ({ onContinue }) => {
        ...
        return (
            <main className="flex-grow flex flex-col justify-center p-4 space-y-6">
                <div className="text-center">
                    <h2 className="text-2xl font-bold">Select Date Range for Analysis</h2>
                    <p className="text-zinc-400 mt-2">The AI will use your nutrition and measurement data from this period to generate a personalized plan.</p>
                </div>
                ...
                <button
                    ...
                    className="w-full bg-amber-500 hover:bg-amber-600 text-black font-bold py-3 px-4 rounded-lg"
                >
                    Continue
                </button>
            </main>
    ...
    return (
        <div className="absolute inset-0 bg-zinc-950 z-50 flex flex-col font-sans text-zinc-200">
            ...
            {step === 'dateSelection' && (
                 <SelectDateRangeStep onContinue={handleDateSelectionContinue} />
            )}
                ...
                <main className="flex-grow overflow-y-auto p-4 space-y-6">
                    ...
                    <div>
                        <textarea
                            ...
                            placeholder="Describe your desired nutrition plan..."
                            rows={3}
                        ...
                        <button 
                            ...
                            className="w-full mt-2 bg-amber-500 hover:bg-amber-600 text-black font-bold py-2 px-4 rounded-lg focus:outline-none focus:shadow-outline transition-colors duration-200 disabled:bg-zinc-600"
                        >
